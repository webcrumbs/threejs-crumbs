	...
	parse: function ( scene ) {
		...
		function GeometryString( g ) {
			...
			if ( g instanceof THREE.SphereGeometry ) {
				...
				'	"type"    : "sphere",',
				'	"radius"  : ' 		 + (g.radius || g.parameters.radius) + ',',
				'	"widthSegments"  : ' + (g.widthSegments || g.parameters.widthSegments) + ',',
				'	"heightSegments" : ' + (g.heightSegments || g.parameters.heightSegments),
				'}'

				];

			} else if ( g instanceof THREE.CubeGeometry || g instanceof THREE.BoxGeometry) {

				var output = [

				'\t' + LabelString( getGeometryName( g ) ) + ': {',
				'	"type"    : "cube",',
				'	"width"  : '  + (g.width || g.parameters.width)  + ',',
				'	"height"  : ' + (g.height || g.parameters.height) + ',',
				'	"depth"  : '  + (g.depth || g.parameters.depth)  + ',',
				'	"widthSegments"  : ' + (g.widthSegments || g.parameters.widthSegments) + ',',
				'	"heightSegments" : ' + (g.heightSegments || g.parameters.heightSegments) + ',',
				'	"depthSegments" : '  + (g.depthSegments || g.parameters.depthSegments),
				'}'
